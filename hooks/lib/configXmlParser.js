/*
Parser for config.xml file. Read plugin-specific preferences (from <openapp> tag) as JSON object.
*/
...
function readPreferences ( cordovaContext ) {
    ...
    // look for data from the <openapp> tag
    var ulXmlPreferences = configXml.widget[ 'openapp' ];
    if ( ulXmlPreferences == null || ulXmlPreferences.length == 0 ) {
        console.warn ( '<openapp> tag is not set in the config.xml. Universal Links plugin is not going to work.' );
        return null;
    }
