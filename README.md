...
- [Useful notes on Universal Links for iOS](#useful-notes-on-openapp-for-ios)
...
```sh
cordova plugin add cordova-plugin-openapp
```

It is also possible to install via repo url directly (**unstable**)

```sh
cordova plugin add https://github.com/eduardokrausme/cordova-plugin-openapp.git
```
...
1. Clone the `xcode6-support` branch of the plugin from the GitHub:
  ...
  git clone -b xcode6-support https://github.com/eduardokrausme/cordova-plugin-openapp.git
  ```
...
2. Go to your applications project and add plugin from the cloned source:
  ...
  cordova plugin add /Workspace/Mobile/CordovaPlugins/cordova-plugin-openapp/
  ```
...
Those preferences are specified inside the `<openapp>` block. For example:
...
1. Create new Cordova application and add Android platform.
  ...
  ```sh
  cordova create TestAndroidApp com.example.app TestAndroidApp
  cd ./TestAndroidApp
...
3. Add `<openapp>` preference into `config.xml`:
...
1. Open `config.xml` and change `<openapp>` block like so:
...
3. Add `<openapp>` preference into `config.xml` (`TestAndroidApp/config.xml`):
...
But if you don't want to... well, there is one way to skip it. You can use [branch.io](https://branch.io) to handle all the SSL/apple-app-site-association stuff for you. How to do that - described in their [documentation](https://dev.branch.io/recipes/branch_universal_links/#enable-openapp-on-the-branch-dashboard). From there you can skip Xcode and SDK integration stuff, because you don't need that.
...
5. Create new Cordova iOS application and add UL plugin:
  ...
  ```sh
  cordova create TestProject com.example.app TestProject
  cd ./TestProject
...
First of all - you need to accept the fact, that Universal Links **doesn't work everywhere**. Some applications doesn't respect them. You can read more in [that article](https://blog.branch.io/ios-9.2-deep-linking-guide-transitioning-to-openapp), section `Universal Links Still Don’t Work Everywhere`.
...
- [How to setup universal links on iOS 9](https://blog.branch.io/how-to-setup-openapp-to-deep-link-on-apple-ios-9)
- [Branch.io documentation on universal links](https://dev.branch.io/recipes/branch_universal_links/#enable-openapp-on-the-branch-dashboard)
- [Where universal links don't work](https://blog.branch.io/ios-9.2-deep-linking-guide-transitioning-to-openapp)
